...
public interface LeaveRepository extends JpaRepository<LeaveEntity, Long> {
	...
	@Query(value = "select empid, sum(amount) as amount, 'BIMONTHLY' as TYPE from employeewellbeing.leave where datesubmit >= (CURRENT_DATE - INTERVAL '60 days')  and  type != 'SICK_LEAVE' group by empid", nativeQuery = true)
